...
static func _tint(kind: String, u: float, v: float, h: float) -> Vector3:
	# Рисунок должен ЛЕЖАТЬ на цвете, а не гасить его. Диапазон узкий нарочно:
	# при широком (0.66…1.0) каждая поверхность темнела на треть, и ночной зал,
	# и без того тёмный, проваливался в сплошную черноту.
	var base := 0.82 + h * 0.24
	match kind:
		"wood":
