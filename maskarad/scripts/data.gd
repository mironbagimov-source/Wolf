...
const TUNE := {
	...
	# ПОДНЯТЬ УПАВШЕГО. Сбитый встаёт сам через девять секунд — товарищ
	# поднимает за три. Это единственная причина, по которой троим людям
	# выгодно держаться вместе, а не разбегаться по прожекторам.
	"revive_time": 3.0,
	"revive_range": 2.0,
	"revive_hp": 0.35,              # доля здоровья, с которой поднялся

	# ГОЛОД ВЫДАЁТ. Раньше голод был только бонусом (на сотне — чужой облик),
	# и вампиру ничто не мешало пересидеть ночь в углу. Теперь у голода есть
	# и обратная сторона: с какого-то предела клыки не убираются, лицо
	# бледнеет, и Кьяра видит это через зал.
	"hunger_tell": 88.0,

	# ПЕРЕНОСКА. Схваченного можно унести — и это то, ради чего на карте
	# вообще есть гримёрки, подсобки и тёмный двор.
	#
	# Раньше захват упирался в кормление на месте: где схватил, там и пил, на
	# глазах у зала. Уносить было нельзя, и все приватные комнаты работали
	# только как приманка — увести человека можно было лишь уговорами.
	#
	# Цена переноски: вдвое медленнее, обе руки заняты (драться нельзя), и
	# жертва вырывается. Шкала вырывания растёт быстрее, когда жертва бьётся,
	# и почти стоит на месте, если она уже обмякла от кормления, — поэтому
	# выгодно сначала отпить, а потом нести.
	"carry_speed": 0.45,            # доля обычной скорости
	"carry_struggle": 100.0,        # шкала вырывания
	"carry_struggle_rate": 17.0,    # в секунду, пока жертва в силах
	"carry_drop_stun": 1.2,         # оба замирают, когда жертва вырвалась

	# РАЗГОВОР. Дальше этого разговор обрывается сам: отошёл — значит закончил.
	# Число одно на всех: и панель разговора, и актёр, который держит
	# собеседника развёрнутым, меряют одним и тем же.
	"talk_range": 4.6,
...
# ---------------------------------------------------------------- обряды
## ОБРЯДЫ: кормление вампира и казнь лича.
##
## Раньше и то и другое было «каналом»: жертву подтягивало на фиксированное
## расстояние, обе стороны стояли столбами, и всё кончалось через две с
## половиной секунды. Это читалось как задержка, а не как сцена.
##
## Обряд — другое. Он ДЛИННЫЙ и ФИКСИРОВАННЫЙ, как захват у дочерей
## Димитреску: начался — идёт до конца, обе стороны в нём заперты, никто
## никуда не идёт и ничего не отменяет. И у каждого он СВОЙ: по тому, как
## именно тебя взяли, видно, кто тебя взял, ещё до того, как ты обернёшься.
##
## `keys` — хореография жертвы В ОСЯХ УБИЙЦЫ: `pos` относительно него (−Z
## перед лицом), `yaw` — куда развёрнута относительно его взгляда. Между
## ключами всё сглаживается, поэтому жертву не дёргает, а ведёт.
const RITUALS := {
	# МОЙРА. Разворачивает спиной к себе, заламывает руку и пьёт сбоку из
	# шеи. Работа без изящества: серп, хват, дело сделано.
	"moira_feed": {
		"time": 5.4, "kill": "drain",
		"keys": [
			{"at": 0.00, "pos": Vector3(0, 0, -1.30), "yaw": 0.0},
			{"at": 0.14, "pos": Vector3(0, 0, -0.62), "yaw": 0.0},      # рывок к себе
			{"at": 0.26, "pos": Vector3(0.10, 0, -0.55), "yaw": PI},    # развернула спиной
			{"at": 0.70, "pos": Vector3(0.10, -0.10, -0.50), "yaw": PI}, # пьёт
			{"at": 0.88, "pos": Vector3(0.05, -0.28, -0.62), "yaw": PI}, # оседает
			{"at": 1.00, "pos": Vector3(0, -0.42, -0.95), "yaw": PI * 0.8}, # отпустила
		],
	},
	# ЛЮЦИУС. Всё то же самое, но как приглашение на танец: берёт за руку,
	# ведёт полукруг, откидывает назад на своей руке и пьёт с запрокинутого
	# горла. Со стороны — фигура из вальса, и в этом весь ужас.
	"lucius_feed": {
		"time": 5.8, "kill": "drain",
		"keys": [
			{"at": 0.00, "pos": Vector3(0, 0, -1.30), "yaw": 0.0},
			{"at": 0.16, "pos": Vector3(0.45, 0, -0.90), "yaw": -0.5},  # повёл в сторону
			{"at": 0.30, "pos": Vector3(0.15, 0, -0.60), "yaw": 0.35},  # довёл полукруг
			{"at": 0.44, "pos": Vector3(0, 0, -0.52), "yaw": 0.0},      # откинул назад
			{"at": 0.78, "pos": Vector3(0, -0.14, -0.52), "yaw": 0.0},  # пьёт
			{"at": 0.92, "pos": Vector3(0, -0.10, -0.70), "yaw": 0.0},  # поставил на ноги
			{"at": 1.00, "pos": Vector3(0, 0, -0.95), "yaw": 0.2},
		],
	},
	# Низший вампир. Ни хвата, ни фигуры — валит и пьёт как придётся.
	"thrall_feed": {
		"time": 4.0, "kill": "drain",
		"keys": [
			{"at": 0.00, "pos": Vector3(0, 0, -1.20), "yaw": 0.0},
			{"at": 0.18, "pos": Vector3(0, 0, -0.58), "yaw": 0.0},
			{"at": 0.80, "pos": Vector3(0, -0.22, -0.55), "yaw": 0.0},
			{"at": 1.00, "pos": Vector3(0, -0.38, -0.90), "yaw": 0.0},
		],
	},

	# КАРЛ. Берёт за горло одной рукой и отрывает от пола. Поднимает НЕ высоко
	# и не может: рука у него на высоте плеча, и горло жертвы должно оказаться
	# именно там. Первая версия задирала жертву на две трети метра — та
	# оказывалась выше руки палача и просто висела в воздухе сама по себе.
	"karl_mori": {
		"time": 4.6, "kill": "mori", "grip": "throat",
		"keys": [
			{"at": 0.00, "pos": Vector3(0, 0.00, -1.35), "yaw": 0.0},
			# Точки — только подводка и бросок. Пока держат за горло, место
			# жертвы считается не отсюда, а по НАСТОЯЩЕЙ кисти палача
			# (`grip: throat`): подбирать углы под кость бесполезно, их
			# сдвигает любая правка позы плеч или корпуса.
			{"at": 0.16, "pos": Vector3(-0.20, 0.06, -0.46), "yaw": 0.0},  # оторвал от пола
			{"at": 0.34, "pos": Vector3(-0.23, 0.10, -0.43), "yaw": 0.0},  # держит на весу
			{"at": 0.66, "pos": Vector3(-0.23, 0.10, -0.43), "yaw": 0.0},  # занёс топор
			{"at": 0.80, "pos": Vector3(-0.20, 0.02, -0.52), "yaw": 0.0},  # удар
			{"at": 1.00, "pos": Vector3(-0.10, 0.00, -0.85), "yaw": 0.25}, # бросил
		],
	},
	# ЛАРА. Бьёт гарпуном в упор, тянет на лине к себе, ставит ногу и
	# выдёргивает. Ни одного лишнего движения — она работает инструментом.
	"lara_mori": {
		"time": 4.8, "kill": "mori",
		"keys": [
			{"at": 0.00, "pos": Vector3(0, 0, -1.70), "yaw": 0.0},
			{"at": 0.18, "pos": Vector3(0, 0, -1.55), "yaw": 0.0},      # выстрел в упор
			{"at": 0.46, "pos": Vector3(0, 0, -0.85), "yaw": 0.0},      # притянула
			{"at": 0.62, "pos": Vector3(0, 0, -0.80), "yaw": 0.15},     # сложилась
			{"at": 0.84, "pos": Vector3(0, 0, -1.00), "yaw": 0.15},     # выдернула
			{"at": 1.00, "pos": Vector3(0, 0, -1.15), "yaw": 0.3},
		],
	},
}

## Какой обряд у кого. Отдельная таблица, чтобы обращённые и гости не
## требовали своей строки: чего нет — берётся общее.
const RITUAL_OF := {
	"moira": "moira_feed",
	"lucius": "lucius_feed",
	"thrall": "thrall_feed",
...
static func feed_ritual(char_id: String) -> String:
	return RITUAL_OF.get(char_id, "thrall_feed")

static func mori_ritual(char_id: String) -> String:
	return RITUAL_OF.get(char_id, "karl_mori")

## Как ведёт себя ЖЕРТВА. Тоже по персонажу: у каждого свой характер, и в
## захвате он виден лучше, чем где-либо ещё.
##
##   fight   — насколько сильно бьётся (доля от общей);
##   hold    — как долго держится, прежде чем обмякнуть (доля обряда);
##   style   — своя манера, её отыгрывает `rig`.
const VICTIM_STYLE := {
	"helga":  {"fight": 1.00, "hold": 0.80, "style": "brace"},
	"jay":    {"fight": 0.95, "hold": 0.70, "style": "thrash"},
	"chiara": {"fight": 0.75, "hold": 0.60, "style": "reach"},
	"civ_medea": {"fight": 0.55, "hold": 0.42, "style": "faint"},
	"civ_maria": {"fight": 0.60, "hold": 0.48, "style": "claw"},
}

static func victim_style(char_id: String) -> Dictionary:
	return VICTIM_STYLE.get(char_id, {"fight": 0.7, "hold": 0.5, "style": "claw"})
...
const ACTIONS := {
	...
	# ответы в разговоре: цифра — реплика. Ряд цифр и NumPad вместе, чтобы не
	# гадать, какой из них у игрока под рукой.
	"answer_1": [KEY_1, KEY_KP_1],
	"answer_2": [KEY_2, KEY_KP_2],
	"answer_3": [KEY_3, KEY_KP_3],
	"answer_4": [KEY_4, KEY_KP_4],
	"answer_5": [KEY_5, KEY_KP_5],
}
