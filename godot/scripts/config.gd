...
## Насколько широко вскрыто тело под конкретную начинку.
##
## r — множитель радиуса разреза, burn — ширина обугленного пояска вокруг
## него (метры). ЭМИ-сердце сажает вокруг себя импульс: кожа не просто
## разрезана, а СНЯТА широким пятном, и края спеклись. Факел жжёт слабее,
## но тоже оставляет ожог. Остальным хватает обычного разреза.
const CIV_IMPLANT_CUT := {
	"emp": {"r": 1.42, "burn": 0.085},
	"flare": {"r": 1.15, "burn": 0.10},
	"cryo": {"r": 1.10, "burn": 0.0},
	"singularity": {"r": 1.22, "burn": 0.05},
	"brood": {"r": 1.30, "burn": 0.0},
}
