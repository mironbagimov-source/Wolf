...
нейросеть — своя на этом же компьютере или Claude по ключу. Правила считает
движок, кубики бросает код — и каждый бросок видно всем за столом.
...
Мастер физически не может «подкрутить» бросок: он вызывает инструмент, движок
...
сервера, ни установки.

Остаётся выбрать в лобби, кто будет вести партию, — см. следующий раздел.
...
уровни. Модель нужна ровно за одним — придумывать историю. Откуда её брать,
...
Локальная модель пока умеет только однофайловая сборка: серверу нужен ключ
Anthropic. Без него игра работает, но мастера придётся отыгрывать человеку.
