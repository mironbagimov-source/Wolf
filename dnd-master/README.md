...
пяти букв дай друзьям, они заходят по нему.

Ключ берётся на https://console.anthropic.com/settings/keys. Без ключа
приложение работает, но мастера придётся отыгрывать человеку.
...
dnd-master/
  ...
  test/              22 теста, включая полный прогон хода мастера
  tools/build-srd.mjs сборка компактной базы SRD
```
...
`DM_MODEL` в `.env`. Там же `DM_EFFORT`: `low` и `medium` отвечают быстрее,
`high` и выше заметнее в сложном бою и интригах.
