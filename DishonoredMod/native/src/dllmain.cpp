...
constexpr int kConfigVersion = 14;
...
//
// Раньше здесь стоял наблюдатель: он ничего не меняет, а значит безопасен как
// умолчание. Но безопасность тут не та, что нужна. Плагин ставится ради
// разведки, и заход в игру стоит вечера; молчаливый наблюдатель, доставшийся
// из-за потерянного или устаревшего конфига, этот вечер тратит впустую — так
// уже случалось. Сбор данных ничего в игре не меняет тоже, зато не пропадает
// зря, если конфиг не доехал.
constexpr char kDefaultMode[] = "harvest";
