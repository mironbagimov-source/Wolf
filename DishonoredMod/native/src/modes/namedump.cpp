...
void NameDumpMode::onDisable() {
    ...
    // Игру закрыли раньше, чем таблица набралась: неполную снять лучше, чем
    // никакой. Но только если режим вообще успел что-то увидеть — выключение
    // сразу после старта не повод снимать пустоту.
    if (events_ < kMinEventsToDump) {
        DMK_INFO("выгрузка имён: выключена, событий было всего %llu — снимать "
                 "нечего", events_);
        return;
    }
    DMK_INFO("выгрузка имён: снимаю таблицу досрочно, событий было %llu", events_);
    dump();
}
